...
$(function () {
    ...
    $("#task-content-inner li").live("click", function () {
        ...
        if(dom.length > 0 && !dom.hasClass("now-nav")){//选择不是同一个菜单项
            ...
            var top_dom= dom.parents(".nav-list");
            if(!top_dom.is(':visible') || $("#myTabs .active").length == 0){
                $("#myTabs .active").removeClass("active");
                $(".nav_wraper .nav-list").hide();
                var top_id = top_dom.attr("id");
                $("#myTabs li[aria-controls="+top_id+"]").addClass("active");
                top_dom.show();
            }
        }else if(dom.length == 0){//没有菜单项
            $("#myTabs .active").removeClass("active");
            $(".nav_wraper .now-nav").removeClass("now-nav");
        }
    	return false;
    });
...
function openapp(url, appid, appname, refresh,dom) {
	...
	$task_content_inner.animate({ "margin-left": marginleft + "px" }, 300, 'swing');
    if(dom && !$(dom).hasClass("now-nav")){
        $(dom).parents(".nav_wraper").find(".now-nav").removeClass("now-nav")
        $(dom).addClass("now-nav")
        var top_dom= $(dom).parents(".nav-list");
        if($("#myTabs .active").length == 0){
            var top_id = top_dom.attr("id");
            $("#myTabs li[aria-controls="+top_id+"]").addClass("active");
        }
    }
